...
LABEL org.opencontainers.image.description "A package that demo's pizza charts"
...
RUN python -m venv /opt/pizza-env
ENV PATH /opt/pizza-env/bin:$PATH
ENV VIRTUAL_ENV /opt/pizza-env

# Set the working directory.
WORKDIR /workspaces/pizza/

# Touch minimal files to allow Poetry to install dependencies.
RUN mkdir -p /root/.cache/pypoetry/ && mkdir -p /root/.config/pypoetry/ && \
    mkdir -p src/pizza/ && touch src/pizza/__init__.py && touch README.md
...
COPY poetry.lock* pyproject.toml /workspaces/pizza/
RUN --mount=type=cache,target=/root/.cache/pypoetry/ \
    poetry install --no-interaction --no-ansi

# Install pre-commit hooks & activate starship.
COPY .pre-commit-config.yaml /workspaces/pizza/
...
ENTRYPOINT ["/opt/pizza-env/bin/poe"]
CMD ["serve"]
